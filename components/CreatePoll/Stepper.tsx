...
export default function Stepper({ steps, currentStep, setCurrentStep }: StepperProps) {
    return (
        <div className="flex flex-row justify-between items-center max-w-2xl m-auto pb-2">
            {steps.map((step, i) => <Step isCurrent={i === currentStep} index={i} setCurrentStep={setCurrentStep} {...step}></Step> )}
        </div>
    );
}
...
function Step({ Icon, name, isCurrent, index, setCurrentStep }: StepProps) {
    return (
        <div className={`flex flex-col justify-center items-center transition-all cursor-pointer select-none ${!isCurrent ? 'text-gray-400 opacity-75' : 'text-cyan-500'}`} onClick={() => setCurrentStep(index)}>
